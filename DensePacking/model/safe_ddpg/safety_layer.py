...
from tensorflow.keras import Model, Sequential, layers, initializers, regularizers, optimizers
...
class Net(Model):
    def __init__(self, in_dim, out_dim, layer_dims, init_bound, initializer, last_activation):
...
class SafetyLayer(Model):
    def __init__(self, env, learning_rate, buffer_size):
        self.env = env
        self.learning_rate = learning_rate
        self.buffer_size = buffer_size

        self.model = Sequential([
			      layers.Input(shape=(self.env.observation_space.shape[0],)),
			      layers.Dense(units=10, activation=tf.nn.relu),
            layers.Dense(units=10, activation=tf.nn.relu),
			      layers.Dense(units=self.env.action_space.shape[0])
		    ], name = 'safetylayer')
